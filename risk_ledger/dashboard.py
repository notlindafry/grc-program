...
# --- brand tokens (SPEC §7). Verified for contrast on the dark surface. -------
BG = "#0f120d"
SURFACE = "#1d231c"
ACCENT = "#7d9b83"          # sage
TEXT = "#e6e4db"
...
MUTED = "#9a988f"
GRID = "#2c342b"
OVER = "#cf8f83"            # --status-over (red)
AT = "#5fc07e"             # --status-at (green)
BELOW = "#cda23e"          # --status-below (amber)
BELOW_TINT = "#ddb95f"     # amber text tint for <13px
...
def _svg(w: int, h: int, body: str, title: str) -> str:
    return (f'<svg viewBox="0 0 {w} {h}" width="100%" role="img" '
            f'aria-label="{_esc(title)}" font-family="Inter, system-ui, sans-serif">{body}</svg>')


def _t(x, y, s, *, size=12, fill=TEXT, anchor="start", weight=400, mono=False) -> str:
    fam = ' font-family="\'Space Grotesk\', Inter, sans-serif"' if mono else ""
    return (f'<text x="{x:.1f}" y="{y:.1f}" font-size="{size}" fill="{fill}" '
            f'text-anchor="{anchor}" font-weight="{weight}"{fam}>{_esc(s)}</text>')
...
def exposure_bar_svg(rows: list[tuple], axis_max: float) -> str:
    ...
    x = lambda v: left + plot_w * min(v / axis_max, 1.0)
    body = [f'<rect x="0" y="0" width="640" height="{h}" fill="{SURFACE}"/>']
    # gridlines + axis value labels (below the plot, above the caption)
    step = nice_ceiling(axis_max / 4)
    v = 0.0
    while v <= axis_max + 1e-6:
        body.append(f'<line x1="{x(v):.1f}" y1="{top}" x2="{x(v):.1f}" y2="{plot_bottom}" stroke="{GRID}" stroke-width="1"/>')
        body.append(_t(x(v), plot_bottom + 12, money(v), size=10, fill=MUTED, anchor="middle"))
        v += step
    for i, (name, low, mean, high, thr, state) in enumerate(rows):
        y = top + i * (row_h + gap)
        cy = y + row_h / 2
        colour = RAG[state][1]
        body.append(_t(left - 10, cy + 4, name, size=12, fill=TEXT, anchor="end"))
        # whisker (5–95)
        body.append(f'<line x1="{x(low):.1f}" y1="{cy:.1f}" x2="{x(high):.1f}" y2="{cy:.1f}" '
                    f'stroke="{colour}" stroke-width="1.5" opacity="0.5"/>')
        # mean bar from 0
        body.append(f'<rect x="{left}" y="{y + 5:.1f}" width="{max(x(mean) - left, 2):.1f}" '
                    f'height="{row_h - 10}" rx="4" fill="{colour}"/>')
        # appetite tick
        if thr <= axis_max:
            body.append(f'<line x1="{x(thr):.1f}" y1="{y + 1:.1f}" x2="{x(thr):.1f}" y2="{y + row_h - 1:.1f}" '
                        f'stroke="{TEXT_STRONG}" stroke-width="1.5" stroke-dasharray="2 2"/>')
    ...
    body.append(f'<line x1="{left}" y1="{cap_y - 4}" x2="{left}" y2="{cap_y + 2}" '
                f'stroke="{TEXT_STRONG}" stroke-width="1.5" stroke-dasharray="2 2"/>')
    body.append(_t(left + 8, cap_y, "appetite line", size=10, fill=MUTED))
    body.append(_t(636, cap_y, "bar = mean residual · line = 5–95% range", size=10, fill=MUTED, anchor="end"))
...
def launch_debt_svg(rows: list[tuple], axis_max: float) -> str:
    ...
    for i, (okr, reported, true) in enumerate(rows):
        y = top + i * (row_h + gap)
        body.append(_t(left - 10, y + 13, okr, size=12, fill=TEXT, anchor="end"))
        # reported (sage) then the undeclared debt (amber) overhang
        body.append(f'<rect x="{left}" y="{y:.1f}" width="{max(x(reported) - left, 1):.1f}" height="16" rx="4" fill="{ACCENT}"/>')
        if true > reported:
            body.append(f'<rect x="{x(reported):.1f}" y="{y:.1f}" width="{max(x(true) - x(reported), 1):.1f}" height="16" rx="4" fill="{BELOW}"/>')
            body.append(_t(x(true) + 6, y + 13, "+" + money(true - reported) + " hidden", size=10, fill=BELOW_TINT))
        body.append(_t(left, y + 27, f"reported {money(reported)} · true {money(true)}", size=10, fill=MUTED))
    body.append(f'<rect x="{left}" y="{h - 12}" width="10" height="10" rx="2" fill="{ACCENT}"/>')
    body.append(_t(left + 15, h - 3, "reported footprint", size=10, fill=MUTED))
    body.append(f'<rect x="{left + 130}" y="{h - 12}" width="10" height="10" rx="2" fill="{BELOW}"/>')
    body.append(_t(left + 145, h - 3, "undeclared debt (diverted in)", size=10, fill=MUTED))
...
def cankicking_scatter_svg(points: list[tuple], x_max: float, y_max: float) -> str:
    ...
    w, h = 640, 264
    px = lambda a: left + (w - left - right) * min(a / x_max, 1.0)
    py = lambda e: (h - bottom) - (h - bottom - top) * min(e / y_max, 1.0)
    body = [f'<rect x="0" y="0" width="{w}" height="{h}" fill="{SURFACE}"/>']
    # gridlines + axes
    for f in (0.5, 1.0):
        gy = py(y_max * f)
        body.append(f'<line x1="{left}" y1="{gy:.1f}" x2="{w - right}" y2="{gy:.1f}" stroke="{GRID}" stroke-width="1"/>')
        body.append(_t(left - 6, gy + 3, money(y_max * f), size=10, fill=MUTED, anchor="end"))
    body.append(f'<line x1="{left}" y1="{top}" x2="{left}" y2="{h - bottom}" stroke="{GRID}"/>')
    body.append(f'<line x1="{left}" y1="{h - bottom}" x2="{w - right}" y2="{h - bottom}" stroke="{GRID}"/>')
    for f in (0.5, 1.0):
        body.append(_t(px(x_max * f), h - bottom + 15, f"{int(x_max * f)}d", size=10, fill=MUTED, anchor="middle"))
    body.append(_t(left, top - 4, "exposure ↑", size=10, fill=MUTED))
    body.append(_t((left + w - right) / 2, h - 6, "age since filed (days) →", size=10, fill=MUTED, anchor="middle"))
    ...
    for age, exp, label in sorted(points, key=lambda p: (py(p[1]), px(p[0]))):
        cx, cy = px(age), py(exp)
        body.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="6" fill="{BELOW}" fill-opacity="0.85" stroke="{SURFACE}" stroke-width="1.5"/>')
        ...
        ly = cy + 3
        while any(not (lx1 < px0 or lx0 > px1) and abs(ly - by) < 13 for px0, px1, by in placed):
            ly += 14
        placed.append((lx0, lx1, ly))
        lead_x = cx - 8 if to_left else cx + 8
        if abs(ly - (cy + 3)) > 6:  # leader line when the label was nudged away
            body.append(f'<line x1="{cx:.1f}" y1="{cy:.1f}" x2="{lead_x:.1f}" y2="{ly - 3:.1f}" stroke="{MUTED}" stroke-width="0.75" opacity="0.6"/>')
        body.append(_t(cx + (-12 if to_left else 12), ly, label, size=10, fill=TEXT, anchor=anchor))
...
def _short(nid: str) -> str:
    return nid.replace("NR-", "").replace("-", " ").title()
...
def _summary(graph: Graph, eng: GraphEngine) -> str:
    ...
    cards.append(
        ...
        f'<b style="color:{BELOW_TINT}">{round(p.p_over_capacity * 100)}% chance</b> of crossing the '
    ...
    tiles = [
        ("Top fixes", ", ".join(_short(r.named_risk.id) for r in over[:3]) or "—",
         f"{len(over)} named risks over appetite"),
        ("Riding a launch", (_short(launch.okr) if launch else "—") if launch else "—",
         (f"+{money(launch.true.mean - launch.reported.mean)} undeclared debt on {launch.okr}" if launch else "no diverted debt")),
...
def _view1(graph: Graph, eng: GraphEngine) -> str:
    ...
    rows = [(_short(r.named_risk.id), r.band.low, r.band.mean, r.band.high, r.threshold, r.state) for r in ranked]
    items = []
    for r in ranked:
        ...
        items.append(
            f'<tr><td>{_dot(r.state)}</td><td class="nm">{_esc(_short(r.named_risk.id))}</td>'
...
def _view3(graph: Graph, eng: GraphEngine) -> str:
    ...
    rows = []
    for h in unhealthy:
        colour = OVER if h.health == "red" else BELOW
...
def _view4(graph: Graph, eng: GraphEngine) -> str:
    ...
    rows = [(_short(d.okr) if d.okr.startswith("NR") else d.okr, d.reported.mean if d.reported else 0, d.true.mean) for d in top]
    # starvation chain
    starved = {}
...
def _view5(graph: Graph, eng: GraphEngine) -> str:
    ...
    srows = "".join(
        ...
        f'<td class="num" style="color:{OVER}">{_esc(r.target_date)}</td><td class="drv">{_esc(r.title)}</td></tr>'
        for r in slipped)
